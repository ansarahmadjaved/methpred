...
fromCpgs = int(input('Starting CPGs :'))
toCpgs = int(input('Ending CPGs :'))
...
models.append(('RandomForest 2', RandomForestRegressor(n_estimators=2, n_jobs=5)))
...
for SelectedCpg in list(CpgsGeneDict.keys())[fromCpgs:toCpgs]:   
    for model in models:        
        ...
        pd.DataFrame(ResultList).to_csv(f'EnsembleModel.{fromCpgs}.to.{toCpgs}.csv')
